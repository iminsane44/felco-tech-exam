...
<div class="container">
  ...
  <form method="POST" action="{{ url('/edit-product/'.$product->id) }}">
    ...
    <select class="form-control" name="category_code">
      ...
      <option value="{{ $category->code }}" {{ $product->category_code == $category->code ? 'selected' : '' }}>{{ $category->code }}</option>
      @endforeach
    </select>

    <div class="form-group">
      <label for="exampleFormControlInput1">Item Code</label>
      <input type="text" name="item_code" value="{{ old ('item_code') ?? $product->item_code }}" class="form-control" placeholder="Item Code">
    </div>
